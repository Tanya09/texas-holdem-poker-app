 ...
 */
  isTwoPair(cards: string[]): boolean {
    ...
    if (pairs === 2) {
      // Get all cards with those values
      this.winningCards = cards.filter(card => pairValues.includes(card.slice(0, -1)));

      // Get the cards (excluding the two pairs)
      ...
      // Get the highest remaining card
      const highestRemainingCard = remainingCards[0];

      // Push the highest remaining card to the winning hand
      this.winningCards.push(highestRemainingCard);
